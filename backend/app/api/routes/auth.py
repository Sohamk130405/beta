...
async def google_login(payload: GoogleLoginRequest, response: Response, db=Depends(get_db)):
    ...
    if not settings.google_client_id:
        raise HTTPException(status_code=500, detail={"error": {"code": "CONFIG_MISSING", "message": "Google client ID not configured"}})
    service = AuthService(db, settings.google_client_id)
    try:
        token_payload = await service.verify_google_id_token(payload.id_token)
    except Exception as exc:
        raise HTTPException(status_code=401, detail={"error": {"code": "AUTHENTICATION_REQUIRED", "message": "Invalid Google ID token"}})
...
async def get_me(request: Request, db=Depends(get_db)):
    ...
    if not session_obj or session_obj.is_revoked or session_obj.expires_at < __import__("datetime").datetime.utcnow():
        raise HTTPException(status_code=401, detail={"error": {"code": "AUTHENTICATION_REQUIRED", "message": "Authentication is required."}})
    ...
    if not user:
        raise HTTPException(status_code=401, detail={"error": {"code": "AUTHENTICATION_REQUIRED", "message": "Authentication is required."}})
    return {"id": str(user.id), "name": user.name, "email": user.email, "role": user.role.value, "profile_image_url": user.profile_image_url}
