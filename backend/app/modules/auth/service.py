...
class AuthService:
    ...
    async def create_session_for_user(self, user: User, expires_at: Optional[datetime] = None) -> tuple[str, Session]:
        raw_token, token_hash = generate_session_token()
        if expires_at is None:
            expires_at = Session.default_expiry()
        session_obj = Session(user_id=user.id, token_hash=token_hash, expires_at=expires_at)
